...
import (
	"fmt"
	"github.com/hex-boost/hex-nexus-app/backend/command"
)
...
func (p *Process) GetCommandLineByName(processName string) ([]byte, error) {
	return p.command.Execute(fmt.Sprintf("wmic", "process", "where", fmt.Sprintf("name='%s'", processName), "get", "commandline"))
}
