...
function Dialog({ children }: { children: React.ReactNode }) {
  ...
  return (
    <DialogContext value={{ innerOpen, setInnerOpen }}>
...
const DialogOverlay = ({ ref, className, ...props }: React.ComponentPropsWithoutRef<typeof DialogPrimitive.Overlay> & { ref?: React.RefObject<React.ElementRef<typeof DialogPrimitive.Overlay> | null> }) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      'fixed inset-0 z-[52] bg-background/40 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
      className,
    )}
    {...props}
  />
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          'fixed left-[50%] top-[50%] z-[52] grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg',
...
function InnerDialog({ children }: { children: React.ReactNode }) {
  const context = React.useContext(DialogContext);
  if (!context) {
    throw new Error('InnerDialog must be used within a Dialog');
  }
  React.useEffect(() => {
...
type InnerDialogContentProps = {
  position?: 'default' | 'bottom' | 'top' | 'left' | 'right';
  draggable?: boolean;
} & React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content>;

const InnerDialogContent = (
  { ref, className, children, position = 'default', draggable = false, ...props }: InnerDialogContentProps & { ref?: React.RefObject<React.ElementRef<typeof DialogPrimitive.Content> | null> },
) => {
  const context = React.useContext(DialogContext);
  if (!context) {
  ...
  return (
    <DialogPortal>
      <DialogPrimitive.Content
        ...
        className={cn(
          'fixed left-[50%] top-[50%] z-[60] grid w-full max-w-lg translate-y-[-0%] gap-4 rounded-lg border bg-background p-6 shadow-lg duration-200',
          isClosingByDrag
...
const InnerDialogTitle = ({ ref, className, ...props }: React.ComponentPropsWithoutRef<typeof DialogPrimitive.Title> & { ref?: React.RefObject<React.ElementRef<typeof DialogPrimitive.Title> | null> }) => (
...
const InnerDialogDescription = ({ ref, className, ...props }: React.ComponentPropsWithoutRef<typeof DialogPrimitive.Description> & { ref?: React.RefObject<React.ElementRef<typeof DialogPrimitive.Description> | null> }) => (
